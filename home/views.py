...
def index(request):
    ...
    round = Round.objects.filter(Q(start__date__range=[start_date, end_date]) & Q(ended__isnull=True)).first()
    return render(request, 'pages/index.html', {'round': round, 'buttons': button_matrix})
...
def get_team_card(request):
    ...
    round_team_instance = get_object_or_404(round_team, team__pk=team_id, round__pk=round_id)
